...
plugin directory. The flow:
    ...
    4. atomically copy every file under that single root folder into the
       installed plugin directory (skipping plugin.json so user customisations
       are not overwritten — the active plugin.json is regenerated by the
       plugin code anyway)
...
GITHUB_REPO = "eliasbruno124/Twitcharr"
...
def apply_update(*, current_version: str, data_dir: str) -> dict[str, Any]:
    ...
    Always preserves existing plugin.json so user-edited settings stay intact
    (the live plugin.py mirrors fields/actions from the manifest, but the
    settings live in the DB so they survive in any case).
    """
    try:
        info = _fetch_release()
    except Exception as exc:
        ...
        for root, _dirs, files in os.walk(src_root):
            ...
            for fname in files:
                if fname in SKIP_FILES:
                    continue
                if fname == "plugin.json" and rel_root == ".":
                    # Keep user's manifest if they tweaked button colors etc.
